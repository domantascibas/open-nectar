...
void Adafruit_GFX::setCursor(int16_t x, int16_t y) {
  cursor_x = x;
  if(gfxFont) {
    cursor_y = y + (int16_t)textsize * (uint8_t)gfxFont->yAdvance;
  } else {
    cursor_y = y;
  }
}
...
//      pc.printf("textsize %d yAdv %d\r\n", textsize, f->yAdvance);
//      pc.printf("first 0x%X last 0x%X\r\n", f->first, f->last);
      cursor_y += 6;//(int16_t)textsize * (uint8_t)f->yAdvance;
    }
