...
void pwr_check(statusDataType_t status) {
    ...
    for (i = 1; i < 16; ) {
        ...
        printf("i: %d , VAL: %d , READ_DATA(): %d , STATUS: %d , GET_STATUS(): %d\n\r", i, *PowerData_getStatusPtr(), r, status, GET_STATUS(status));
        if (GET_STATUS(status) != ((i & (1UL << status)) >> status)) {
            printf("[ERROR] GET_STATUS(): %d != (%d << %d) >> %d\n\n\r", GET_STATUS(status), i, status, status);
            return;
        }
        i *= 2;
    }
...
void pwr_setStatus(statusDataType_t status) {
    ...
    if (GET_STATUS(status) != 1) {
        printf("[ERROR] SET_STATUS(): %d != 1\n\n\r", GET_STATUS(status));
        return;
    }
    CLEAR_STATUS(status);
    if (GET_STATUS(status) != 0) {
        printf("[ERROR] GET_STATUS(): %d != 1\n\n\r", GET_STATUS(status));
        return;
    }
    printf("VAL: %d , STATUS: %d , GET_STATUS(): %d\n\r", *PowerData_getStatusPtr(), status, GET_STATUS(status));
    printf("\n\r");
