...
static bool u1wire_search(u1wire_state_t *state, u1wire_rom_code_t *rom_code) {
    ...
    do {
        ...
        if (id_bit != cmp_id_bit) {
            ...
            if (id_bit_number < state->last_discrepancy) {
                search_direction = (rom_code->bytes[rom_byte_number] & rom_byte_mask) > 0;
            } else {
...
u1wire_obj_t *u1wire_enumerate(u1wire_enumerate_t *info) {
