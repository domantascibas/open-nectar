...
enum {
    DS18B20_BITS_PER_DEGREE = 8
};

enum {
    DS18B20_COUNTS_PER_DEGREE = (1 << 8)
};

enum {
    DS18B20_TEMP_BITS = 12,
    DS1820_TEMP_BITS = 9
};

/* Round to the nearest degree.  */
#define DS18B20_TEMP_INT(TEMP) (((TEMP) + DS18B20_COUNTS_PER_DEGREE / 2) >> DS18B20_BITS_PER_DEGREE)
#define DS18B20_TEMP_DOUBLE(TEMP) ((TEMP) / (double)DS18B20_COUNTS_PER_DEGREE + 0.5)
...
int8_t ds18b20_temp_conversion_start(u1wire_t dev);
...
uint8_t ds18b20_temp_ready_p(u1wire_t dev);
...
int8_t ds18b20_temp_read(u1wire_t dev, ds18b20_temp_t *ptemp);
...
otherwise NULL  */
u1wire_t ds18b20_init(u1wire_obj_t *dev);
