...
namespace esp {
  ...
  void send_message() {
    ...
    //   mainBoardError.get_errors(),
      ...
      rtc,
      (deviceOpMode_isReset() ? COMMS_TRUE_VALUE : COMMS_FALSE_VALUE),
      (deviceOpMode_isPairing() ? COMMS_TRUE_VALUE : COMMS_FALSE_VALUE),
      (deviceOpMode_isBoostOff() ? COMMS_TRUE_VALUE : COMMS_FALSE_VALUE),
  ...
  if(esp::is_message_valid(state)) {
    ...
    if(state.is_configured == COMMS_TRUE_VALUE) {
      ...
      if((nectar_contract::HeaterMode)espData.heater_mode != nectar_contract::Boost)
        deviceOpMode_setBoostOff(false);
